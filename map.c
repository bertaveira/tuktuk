...
void modeC(map *mp, FILE *fpw){
  ...
  for( i = 0; i<mp->nPoints; i++) {
    ...
    for (j = i+1; j<mp->nPoints; j++) {
      ...
      if(adj[i][j] == NULL){  // no possible path
        printerror(mp, fpw);
  ...
  newprintpoints(fpw, 1, mp, 0, fim[0], fim);
  fprintf(fpw, "\n");
  // free all
...
void newprintpoints(FILE *fpw, int count, map *mp, int pos, list *lt, list **blist){
  int cost;

  if (lt->next != NULL) {
    newprintpoints(fpw, count+1, mp, pos, lt->next, blist); // recursive call
  } else if(pos+1 < mp->nPoints){
    newprintpoints(fpw, count+1, mp, pos+1, blist[pos+1], blist); // recursive call
  } else{
    if (((node *)(lt->item))->org[0] != -1){
      fprintf(fpw, "%d\n", count); // print number of points ate the end of the recurssion
    }else{
...
list ** getblistlist(map * mp, int vect[mp->nPoints], list ***adj){
  list **out = (list**)malloc(sizeof(list*)*(mp->nPoints-1));
  int i = 0;

  while(i < mp->nPoints-1){
    out[i] = adj[vect[i]][vect[i+1]];
    if(adj[vect[i]][vect[i+1]] == NULL){
      out[i] = reverseList(adj[vect[i+1]][vect[i]], mp, vect[i]);
    }
  }
  return out;
}
